...
const main = async () => {
  ...
  try {
    ...
    const candles = await UPBIT_SERVICE.getCandles(USER.market, token);
    const targetMarkrt = await identifySurgingCoin(candles.data);
    if (targetMarkrt.success) {
      const orderbook = await UPBIT_SERVICE.getOrderBook(USER.market, token);
